...
ไม่มีการตรวจเครื่องหมายคำพูดปิด ตัวตรวจไม่รายงานช่องว่างท้ายบรรทัด ไม่ว่ากี่ช่อง (เป็น hard break ของ Markdown) และ `--fix` ไม่ยุบ ไม่เพิ่ม และไม่ย้ายช่องว่างธรรมดา แก้เฉพาะอักขระระดับ error ที่ระบุไว้ข้างบน คือเปลี่ยน ำ ที่เขียนแยกสองอักขระให้เป็น U+0E33 ลบ zero-width space ลบ BOM ทิ้ง เปลี่ยน NBSP ให้เป็นช่องว่างธรรมดา (คงไว้เป็น U+00A0 เมื่อใช้ `--genre ui`) และแก้อักขระเต็มความกว้างที่หลุดมาจากแหล่ง CJK — เปลี่ยนตัวอักษรละติน ตัวเลข หรือเครื่องหมายวรรคตอนแบบเต็มความกว้างให้เป็นรูป ASCII ปกติ และเปลี่ยนช่องว่างเต็มความกว้าง จุลภาคจีน (`、`) หรือมหัพภาคจีน (`。`) ให้เป็นช่องว่างธรรมดาหนึ่งช่อง (หรือตัดทิ้งถ้ามีช่องว่างอยู่ติดกันแล้ว)
...
ค่าเริ่มต้นของตัวตรวจรับรู้ไวยากรณ์ Markdown จึงไม่นับ fenced block code span และช่องว่างจัดตำแหน่งในตารางเป็นเนื้อความ
